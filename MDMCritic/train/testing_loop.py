...
class TestLoop:
    ...
    def run_loop(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for motion, cond in self.data:
                ...
                # print(f'Starting step {self.step}, save_interval is {self.save_interval}')
                ...
                if self.step % self.save_interval == 0:
                    ...
                    if self.do_sample_when_eval:
                        self.sample_when_eval(0)
                        self.sample_when_eval(1)
                        self.sample_when_eval(2)
                        self.sample_when_eval(3)
                        self.sample_when_eval(4)
    ...
    def my_eval_critic_condbatch(self):
        ...
        print(f"dist_util.dev is {dist_util.dev()}")
...
# critics is a tensor with shape [x]. critic-batch-cond are connected pair. I want to sort and reorder them according to critic value from large to small. how?
        ...
        sorted_critic_list = [critics[i] for i in sorted_indices]
        ...
        sorted_cond = merge_conds(sorted_cond_list[:1190])
        ...
        random_indices = torch.randperm(1190)
        random_batch_list = [batch_list[i] for i in random_indices]
        random_cond_list = [cond_list[i] for i in random_indices]
        random_critic_list = [critics[i] for i in random_indices]

        random_batch_list = random_batch_list[:238]
        random_cond_list = random_cond_list[:238]
        random_critic_list = random_critic_list[:238]

        self.evalrandombatch = torch.cat(random_batch_list)
        self.evalrandomcond = merge_conds(random_cond_list)



    def sample_when_eval(self, id):
        take_batch_size = 238
        if id == 0:
...
# eval that batch.
        if self.dataset in ["humanact12"]:
            ...
            # self.evalrandombatch = self.evalrandombatch.to(dist_util.dev())
            eval_dict =  eval_humanact12_uestc.evaluate(
                ...
                single_batch=True, batch_size=take_batch_size, motion=batch, sample=self.evalfullbatch, model_kwargs=self.evalfullcond, gt_kwargs=cond)
            print(f"fid between 0 and batch-{id}")
            val =  np.array(eval_dict["feats"]["fid_batch_gen"]).astype(float).mean()
            print(f"fid_batch_gen is {val}")           
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            if self.use_kl_loss:
                ...
                if self.pre_sample is not None:
                    ...
                    kl_loss = normal_kl(pre_mean, pre_logvar, curr_mean, curr_logvar)  
                critic_loss = critic_loss + 0.4 * kl_loss
                if self.use_wandb:
