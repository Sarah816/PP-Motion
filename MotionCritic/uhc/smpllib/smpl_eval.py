# This code is based on https://github.com/nicolasugrinovic/multiphys
...
def compute_phys_metrics(pred_vertices, converter=None, key=None, args=None, n=None, use_wbpos=False, use_smpl_gt=False):
    '''
    Input pred_vertices: shape [num_frames=60, 6890, 3]
    '''
    res_dict = {}
    info = {}
    ...
    floating = np.mean(compute_floating(pred_vertices, info))

    res_dict["penetration"] = pent
    res_dict["skate"] = skate
    res_dict["float"] = floating
...
def compute_ground_penetration(vert, info):
    '''
    Compute average penetration depth (mm)
    '''
    # print("compute ground penetration")
    # vert: torch [60, 6890, 3]
    pen = []
    for vert_i in vert:
...
def compute_floating(vert, info):
    '''
    Compute average floating height (mm)
    '''
    # print("compute floating")
    flo = []
    for vert_i in vert:
        min_z = np.min(vert_i[:, 2].cpu().numpy())
        if min_z > 0:
            float = min_z * 1000
        else:
            float = 0.0
...
def compute_skate(vert, info):
    '''
    Compute average "skating length" (mm)
    skating length: average magnitude of the displacement (on x-axis and y-axis) for penetrating vertices
    '''
    # print("compute skate")
    skate = []
